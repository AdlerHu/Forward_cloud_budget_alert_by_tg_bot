...
function notify_and_record_alert_mail(db_spreadsheet_id, db_sheet_name, alert_tag, destination_label, tg_token){
  ...
  threads.forEach(function(thread) {
    ...
    switch (alert_tag){
      case 'aws_alert':
        cloud = 'AWS';
        uid = get_aws_uid(body);
        break;
      case 'gcp_alert':
        cloud = 'GCP';
        uid = get_gcp_uid(body);
        break;
      case 'azure_alert':
        cloud = 'Azure';
        uid = get_azure_uid(body);
        break;
      case 'ali_alert':
        cloud = '阿里雲';
...
function deal_with_credit_alert_email(db_spreadsheet_id, db_sheet_name, alert_tag_list, destination_tag, tg_token){
  ...
  // 依照 AWS -> GCP -> Azure -> 阿里 -> 騰訊 -> 華為 的順序，一次處理一家雲的告警
...
function main(){
  ...
  // 原始告警信標籤
  const alert_tag_list = ['aws_alert', 'gcp_alert', 'azure_alert', 'ali_alert', 'tencent_alert', 'huawei_alert'];
